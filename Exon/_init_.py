...
# When Exon is started through __main__.py using exec(), this file may be
# executed without being registered as the ``Exon`` package in sys.modules.
# Register the current module so internal modules using ``from Exon import ...``
# resolve correctly during startup.
sys.modules.setdefault("Exon", sys.modules[__name__])
import time
...
# Asyncio event loop and AioHttp Session
try:
